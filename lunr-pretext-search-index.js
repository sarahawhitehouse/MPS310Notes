...
},
{
  ...
  "number": "1.1.10",
  "title": "",
  "body": "The complement of a simple graph , which we will denote , and is sometimes written , is the graph with the same vertex set as , but if and only if ; that is, there is an edge between and in if and only if there is not an edge between and in . "
},
{
...
},
{
  ...
  "number": "1.1.16",
  "title": "",
  "body": "The complete bipartite graph  is the graph with red vertices and blue vertices, and an edge between every red vertex and every blue vertex.  "
},
{
...
},
{
  ...
  "number": "1.3.3",
  "title": "",
  "body": "An isomorphism of simple graphs is a bijection between their vertex sets that preserves the number of edges between vertices. In other words, and are adjacent in if and only if and are adjacent in . "
},
{
...
},
{
  ...
  "body": "Counting isomers of  We illustrate both of these methods. We first illustrate by length of the longest path of carbons.  Chain length six: Since we've used all carbons then there are no more choices, and there is only one such isomer.  Chain length five: We need to add one more carbon. We can't add it to either of the end carbons, because then we'd have a path of length 6. We can add it to the central of the two chains, or alternatively to one either side of central -- the last two trees are isomorphic, giving us two possibilities.  Chain length four: We need to add two more carbons. Again, they can't be added to either of the end carbons, or we'd have a longer chain length. Therefore, they most be added to the two central carbons. One case is that we add one carbon to each of the two central carbons. Alternatively, we could add both the carbons to the same \"central\" carbon; reversing the order of the chain is a symmetry that interchanges the two central atoms. We could add each carbon directly to the existing carbon in the chain, or we could add them one after the other making a chain of length two. However, the resulting graph would have a chain of length 5 and already be counted. Thus, there are two possibilities here.  Chain length three: We need at add three more carbons, and there's only one central carbon to attach them to. We can't add them all directly to this central carbon, as that would create a carbon of degree 5. On the other hand, once we add a chain of length longer than one to this central carbon we'd have a path of length 4 or greater.  Thus, we see there are five isomers of . Alternatively, we could organize our search by deleting the hydrogens, and then considering the degrees of the resulting carbon-carbon graph.   Degree at most two: If the resulting tree only had carbons of degree at most two, then it would have to be the path graph , and so we only have one possibility here.  One vertex of degree three: If the resulting graph had exactly one carbon of degree three, that vertex and its three neighbours would account for 4 of our 6 carbons, and so we'd have to add two more. We couldn't add them directly to the same vertex, as that would create a second vertex of degree three. So, they could either be added as a chain of length two to one of the leaves of the existing graph, or they could be added to two separate leaves. Drawing these graphs we see they're not isomorphic, and so we have two possibilities here.  Two vertices of degree three: If we have two vertices of degree three, one sees they'd have to be adjacent to each other, resulting in one possibility.  Vertex of degree four: A vertex of degree 4 and its four neighbours would account for all but one of the carbons. We could add that carbon to any of the leaves, and get one more possiblity.    Isomers of    Since carbons only have degree 4, the tree with six vertices where all are connected to a central vertex isn't allowed, and we have found all the isomers.  "
},
{
...
},
{
  ...
  "number": "2.1.2",
  "title": "Walk.",
  "body": "Walk A in a graph is a sequence   where the are vertices, the are edges, and the edge goes between vertices and .  We say that the walk is between vertices and  "
},
{
...
},
{
  ...
  "number": "2.1.8",
  "title": "",
  "body": " If the edges of the walk are all distinct, we call it a trail .  If the vertices of the walk are all distinct (except possibly the start and end vertices), we call the walk a path . The exception is to allow for the possibility of closed paths. "
},
{
...
},
{
  ...
  "number": "2.2.2",
  "title": "",
  "body": "Let be a graph. An Eulerian cycle is a closed walk that uses every edge of exactly once.  If has an Eulerian cycle, we say that is Eulerian .  If we weaken the requirement, and do not require the walk to be closed, we call it an Euler walk, and if a graph has an Eulerian walk but not an Eulerian cycle, we say is semi-Eulerian . "
},
{
  "id": "thm_eulerian",
  "level": "2",
...
},
{
  ...
  "number": "2.3.5",
  "title": "",
  "body": "Suppose that is bipartite and Hamiltonian, with red vertices and blue vertices. Then .  Consider a Hamiltonian path in . Since every edge is between a red and blue vertex, the vertices in the path must alternate between red and blue. Considering every other edge of the cycle pairs each red vertex with a blue vertex, and hence . "
},
{
  ...
  "number": "2.3.6",
  "title": "",
  "body": "The Petersen Graph is not Hamiltonian.  A picture of the Petersen graph can be found in Figure . Of course, a case-by-case analysis of possibile Hamiltonian cycles is possible. The number of cases can be reduced by using symmetries of the Petersen graph. Instead, for variation and to illustrate a different proof technique, we will use another method.  Assume for contradiction that the Petersen graph is Hamiltonian, and draw the ten vertices around the cycle. The Hamiltonian cycle uses 10 of the 15 edges in the Petersen graph, and so there must be 5 more edges, with each vertex incident to one of them, as in the Petersen graph every vertex has degree 3.  Let's analyse where else the edge adjacent to could go. It can't go to itself, as the Petersen graph has no loops, and it can't go to or as the Petersen graph has no multiple edges. If it went to or it would make a three cycle, which the Petersen graph doesn't have, and if it went to or , there'd be a four cycle. Hence, the only options are is adjacent to or . By reversing the direction of the Hamiltonian path, and are equivalent, and there are only two cases.  But the same analysis holds for every vertex: the extra edge at any vertex can either go to the opposite side of the circle, or be \"off by one\" and skip three vertices in either direction.  We now claim that not all the extra edges can go \"directly across\" -- there must be at least one edge that's off by one. If all the extra edges went directly across, then would go to , and would go to , and would be a 4 cycle.  Hence, without loss of generality we may assume that the extra edge at is . Let us then consider the extra edge at . It can't go directly across, as that is which already has its extra edge. Hence it must be off by one, and go to either or . But either way we get a four cycle: either , or . Hence, we have a contradiction, and the Petersen graph cannot be Hamiltonian.  "
},
{
...
},
{
  ...
  "number": "3.1.4",
  "title": "Prüfer code.",
  "body": "Prüfer code If we record the edges of a tree as in the Pruning Algorithm, the first numbers appearing in the top row give the Prüfer code of "
},
{
...
{
  "id": "s_graphalgorithms_min-wt-span-2-2",
  "level": "2",
  "url": "s_graphalgorithms_min-wt-span.html#s_graphalgorithms_min-wt-span-2-2",
  "type": "Definition",
  "number": "3.2.1",
  "title": "Spanning tree.",
  "body": "Spanning tree A spanning tree of a graph is a subgraph that is a tree and contains all the vertices of . "
},
{
...
{
  "id": "s_graphalgorithms_min-wt-span-2-6",
  "level": "2",
  "url": "s_graphalgorithms_min-wt-span.html#s_graphalgorithms_min-wt-span-2-6",
  "type": "Problem",
  "number": "3.2.3",
  "title": "",
...
},
{
  ...
  "number": "3.2.8",
  "title": "",
  "body": " It is definitely worthwhile to have special algorithms to find minimum weight spanning trees. Recall from that a graph on vertices can have as many as spanning trees. Even if we only want to find a minimum weight spanning tree, we would still have to find the weight of each of these trees and then identify the least weight. This is not a practical approach for even small values of .  "
},
{
...
{
  "id": "subs_graphalgorithms_longest-path-acyclic-sort-2",
  "level": "2",
  "url": "s_graphalgorithms_longest-paths.html#subs_graphalgorithms_longest-path-acyclic-sort-2",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "acyclic "
...
},
{
  ...
  "number": "3.6.1",
  "title": "",
  "body": " A graph where nearest neighbour struggles   "
},
{
...
{
  "id": "s_graphs_on_surfaces_intro-4-2",
  "level": "2",
  "url": "s_graphs_on_surfaces_intro.html#s_graphs_on_surfaces_intro-4-2",
  "type": "Definition",
  "number": "4.1.2",
  "title": "",
...
{
  "id": "s_graphs_on_surfaces_intro-4-6",
  "level": "2",
  "url": "s_graphs_on_surfaces_intro.html#s_graphs_on_surfaces_intro-4-6",
  "type": "Theorem",
  "number": "4.1.3",
...
{
  "id": "s_graphs_on_surfaces_intro-4-11",
  "level": "2",
  "url": "s_graphs_on_surfaces_intro.html#s_graphs_on_surfaces_intro-4-11",
  "type": "Theorem",
  "number": "4.1.4",
  "title": "",
  "body": "is not planar   First let's name the vertices of : let the vertices be the blue circle vertices, and be the red rectangle vertices. Then the path is a Hamiltonian cycle, and so if were planar it would be drawn as a circle in the plane, as shown below:  The Hamiltonian cycle in    This contains 6 of the 9 edges of ; we need to add the edges and . The edge could be drawn inside the circle or outside, suppose we draw it inside, as shown below, with the added edge dashed.  Adding inside   Then on the inside of the circle, and are on different sides of the line , and so the edge connecting them must go outside the circle. The added edge could go around the right of the circle, as shown below here:  Adding inside   or around the left, as shown here:  Adding inside   But now and are different sides of inside the circle, and on different sides of outside the circle, and so cannot be connected without making two edges cross.  If we had began by drawing outside the circle, then we would have had to draw inside the circle, and had the same problem with being able to draw the last line; as shown here:  Adding inside   "
},
{
...
},
{
  ...
  "number": "4.2.2",
  "title": "",
  "body": "  Let be any point. Then is homeomorphic . That is, there are continuous functions and such that is the identity on , and is the identity on .    One way to visualize this is imagine the sphere as being made from very flexible clay. If we poke a small hole in the top of the sphere, we could stick our fingers in and make the hole larger, and gradually stretch and bend and reform for the sphere to be a flat disk, which could be stretched to be the whole plane, in the same way the tangent function maps the interval to the whole real line   Alternatively, one could use stereographic projection , as shown in Figure . Draw in as the unit sphere at the origin, and let be the north pole of the sphere. Stereographic projection gives a bijection between (the sphere minus the north pole) to the plane, as follows: for any point the line through and must meet the -plane at one point. On the other hand, any line through and a point on the -plane must meet the sphere at one other point.   Stereographic projection    Accepting that is minus one point, we see that we can draw a given graph on if and only if we can draw on : if we draw it on , we can view the as a small patch of . And if we have a drawing on , there must be at least one point on that isn't in the drawing of , and doing stereographic projection from that point gives a drawing of on the plane .   "
},
{
...
{
  "id": "s_graphs_on_surfaces_planarity_algorithm-4-7",
  "level": "2",
  "url": "s_graphs_on_surfaces_planarity_algorithm.html#s_graphs_on_surfaces_planarity_algorithm-4-7",
  "type": "Algorithm",
  "number": "4.2.5",
  "title": "The planarity algorithm for Hamiltonian graphs.",
  "body": "The planarity algorithm for Hamiltonian graphs Suppose that is Hamiltonian, and is a Hamiltonian cycle. Then is planar if and only if Cross( ) is bipartite. "
},
{
  ...
  "type": "Example",
  "number": "4.2.6",
  "title": "The complete graph <span class=\"process-math\">\\(K_5\\)<\/span> is not planar.",
  "body": "The complete graph is not planar  Let's apply the planarity algorithm to the complete graph . Let's label the vertices 1-5, and take our Hamiltonian cycle to be 123451, which we've drawn as the outside pentagon in the following figure: The graph  Since has edges, there are 5 edges that aren't used in , namely 13,14, 24, 25, and 35. So Cross( ) will consist of these five vertices. We see that 13 will be adjacent to 24 and 25, since these edges would cross if drawn inside, but 13 is not adjacent to 14 or 35, since these edges would cross 13 if drawn on the same side of the circle, as illustrated in the next figure.  The edges 13 does and does not cross   Similar considerations with the other edges show that Cross( ) is the following graph, which is isomorphic to a five cycle.  The graph Cross( )   In particular, Cross( ) is not bipartite. Hence, by the planarity algorithm for Hamiltonian graphs, we see that is not planar. "
},
{
  "id": "s_graphs_on_surfaces_planarity_algorithm-4-11",
  "level": "2",
  "url": "s_graphs_on_surfaces_planarity_algorithm.html#s_graphs_on_surfaces_planarity_algorithm-4-11",
  "type": "Example",
  "number": "4.2.10",
...
},
{
  ...
  "number": "4.3.5",
  "title": "Subdivision.",
  "body": "Subdivision We say that is a subdivision of if is obtained from by adding some vertices of degree two in the middle of some of the edges of . We call such an extra vertex a subdivision vertex . "
},
{
...
},
{
  ...
  "number": "4.3.7",
  "title": "",
  "body": "If is a subdivision of a nonplanar graph , then is nonplanar. Suppose that was planar, and draw it in the plane. Then erase the vertices of degree 2 we added when we subdivided , merging the edges on either side to one. We obtain a planar drawing of , a contradiction, and so must not have been planar. "
},
{
  ...
  "number": "4.3.8",
  "title": "Kuratowski’s Theorem.",
  "body": "Kuratowski's Theorem  A graph is nonplanar if and only if has a subgraph that's a subdivision of or .   We will only prove one direction: that if has such a subgraph, then is nonplanar; the other direction is more difficult.  Suppose that is a subgraph of that is subdivision of or . Since we've proven and are nonplanar, we know is nonplanar by . Now since is a subgraph of and we know is nonplanar, we know is nonplanar by .  "
},
{
  ...
  "body": "The Petersen graph isn't planar   The Petersen graph, labelled    Let us use Kuratowski's Theorem to prove that the Petersen graph isn't planar; has a drawing of the Petersen graph with the vertices labeled for reference. Since the Petersen graph is regular of degree three, we know that it can't have a subgrpah that's a subdivision of , as it would need to have some vertices of degree 4 or larger.  It makes sense to attempt a greedy algorithm -- in the standard drawing of the Petersen grpah, pick the very top vertex 1 to be \"red\" and the three vertices adjacent to it to, 2, 5, and 6, to be \"blue\". We need two more red vertices. All vertices left are adjacent to exactly one blue vertex, so from a greedy point of view there is no preference for which vertex we pick to be the next red vertex. Let us pick 9 to be another red vertex. Then it is connected directly to blue vertex 6, but we must find paths from 9 to 2 and 5. We could, for instance, take the path 9-4-3-2 to connect to 2, but that would use two vertices up while the path 9-7-2 only uses one extra vertex, and so seems better. Then we can connected 9 to 5 through vertex 4, and vertex 9 has been connected to all the blue vertices.  Now, we need to choose one more vertex to be a red vertex, and the vertices we haven't used are 3, 10, and 8. If we tried to make 3 the last red vertex we run into a problem: we need to connect vertex 3 to three other vertices, but one of the three edges out of vertex 3 goes to vertex 4 which is being used as a subdivision vertex and so we can't visit it again. Hence, we only have two possible paths out of 3, and will ever only be able to connect it to two blue vertices. A similar problem occurs if we try to make 10 the last red vertex -- it's adjacent to the vertex 7 used as a subdivision vertex. The remaining choice is vertex 8, which works, as shown in the following diagram.   A subdivided in the Petersen graph    The red and blue vertices of the subdivided are shown as squares\/circles, and the edges of the subdivided are colored thick -- only the dotted edges 7-10 and 3-4 of the Petersen graph are not used in the subgraph.  "
},
{
  ...
  "body": " Drawing Graphs on Other surfaces  We saw, using stereographic projection, that being able to draw a graph on the sphere is the same as being able to draw the graph on the plane. In this section we will discuss drawing graphs on other surfaces -- the torus and the Möbius strip we will discuss in detail, though similar ideas work for any surface. We need a way to represent such graphs on a piece of paper, for use in a book (or on the exam, say). Many of the results from the rest of this chapter (Kuratowski's theorem, Euler's theorem) have analogues for other surfaces, but theses are beyond the scope of this module.  Motivation and culture: Manifolds and Surfaces  In this short subsection we are going to be slightly informal. The goal is simply to motivate this section about drawing graphs on surfaces other than the sphere, and to give a motivating problem that we will solve in the next section on Euler's Theorem.  People used to think that the earth was flat, because if you can't see the whole thing, but can just look at just one little patch of it around you, it looks like a piece of . Formally, mathematicians say \"locally homeomorphic to \" to mean that you can't tell it apart from by just looking at a small piece of it.  To complement the familiar earth idea of the earth being round, we give a few more shocking thought-experiment examples. The first is: how do we know the earth is round? I, personally haven't been to space, haven't been all over the world. Maybe there's a giant tunnel running from the south pole to the north pole, and the earth is really a torus (the surface of a donut or a bagel).  The idea of the earth being the surface of the torus probably seems absolutely ridiculous, but it happens \"by accident\" in videogames. In old games like Pacman or Asteroids, the game takes place on one computer screen, but to keep it from having edges and corners the designers made the game \"wrap around\" -- if anything goes off the right edge of the screen, it comes back at the corresponding spot on the left edge of the screen, and similarly anything that goes off the top of the screen comes back on the bottom of the screen.  Many other video games, like early ones in the Final Fantasy series, use essentially the same process to model the surface of a planet. We claim that any of these videogame worlds are actually the surface of a torus. Instead of a computer screen, let's put the world on a very flexible flat sheet. To get the left and right edges to match up, we can curl the screen up into a cylinder -- going off one edge takes us around the back of the cylinder. Now, our cylinder has two circular boundaries, coming from the top and bottom of the screen, and to get these to match up we can bend our cylinder up and glue these together to make a torus.  Finally, we can step up a dimension, we generally assume that the three dimensional space we live in is , but what evidence do we have for that? Maybe it's got some different shape, and if we could fly for untold light years in one direction we'd come back to the earth from a different direction! I can recommend Janna Levin's popular science book \/ memoir \"How the Universe got its Spots\" for an account of how physicists studied whether patterns in the cosmic microwave background radiation could have been created by the universe being a shape other than .  That discussion may feel out of place; its purpose was to motivate the following definition, which we will then apply to graph theory.  An - dimensional manifold is a space that is locally homeomorphic to . A surface is a two dimensional manifold.    Graphs on the Torus  The torus is another word for the surface of a doughnut. There are some graphs that can't be on drawn on the sphere, but can be drawn on a torus. But we need a way to represent drawing graphs on the torus just using a normal sheet of paper -- it would be awkward and impractical to hand every student an innertube or a doughnut at the exam to hand in with their papers.  We will do this by copying the videogame worlds we saw in the introduction. Draw a square to represent the videogame screen, and then draw the graph inside the square, with the added condition that if while drawing an edge of the graph we hit the border of the square, we continue the edge at the same point of the opposite side of the square. To make clear what we're doing, it is useful to draw arrows on the edges of the square to indicate how they are being identified -- we put one arrowhead, pointing the same direction, on the left and right edges, to indicate that they are being identified, with the tip end of one edge being matched to the tip end of the other, and the tail end being matched to the tail end of the other. We draw two arrowheads on the top and bottom edges, also pointing the same direction.  and drawn on a torus  This whole process is illustrated in , where the complete graphs on five and six vertices are drawn on the torus. We end with a few short tips and tricks for trying to draw graphs on surfaces.  The first is that it can be complicated to keep track of edges that wrap around the sides of the torus. If lots of them are wrapping around, it can be easy to lose track of which edges are connecting to which others on the opposite side. One way to make this clearer is to label the crossing point on one side letters or numbers in order, and then on the opposite side label with the letters and numbers going in the same direction as indicated by the arrow. Then you know that the edge that crosses at letter , say, on one side, picks up at on the other edge.  Another idea is to try to minimize the number of such crossings, and draw as much of the graph as possible in the center of the square, and only a few edges wrapping around the torus. One heuristic to follow is to work like we did with the planarity algorithm for Hamiltonian graphs, and try to draw a large cycle as a cycle in the square. Then, connect as many edges and extra vertices as possible through the centre of the cycle, and only when you've ruled that out try to wrap edges around the torus. This is the approach taken in the drawing of in ; the outside house is a Hamiltonian cycle, two edges could be drawn in the centre of the house, and then the remaining three edges have no choice but to be drawn wrapping around the torus.  The drawing of uses another trick that is worth explaining. It begins with the drawing of in black, and adds new stuff in red. It appears that four new vertices have been added, one at each corner of the square. But actually, when the edges are identified, the four corners of the sphere get identified into one point on the torus. (Check this by visualizing what happens if you folded the paper up!). Placing one of the vertices on this graph on the corner can be useful because then edges drawn from this vertex will not need to wrap around the sides of the square.   Möbius strip  If you take a piece of paper and roll it up, identifying the two edges in the usual way, you obtain a cylinder. But if instead you identify the edges in the opposite way, you obtain a different surface called the Möbius strip . The Möbius strip is famous for only having one side. In the drawing shown below, if one of the ants walks all the way around the strip, when it returns to where it starts it will be on the opposite side of the strip. Similarly, whereas a cylinder has two boundary cirlces, the Möbius strip only has one.  M.C. Escher's Möbius Strip II   The fact that the Möbius strip only has one boundary circle has the following surprising consequence, that makes a great \"party trick\". This is to make a Möbius strip by taping up a strip of paper, and the cut it down the very middle of the strip -- you wind up not getting two pieces of paper, but just one!  To actually represent graphs drawn on the Möbius strip, we work similarly to what we did for the torus; we draw a square, and then we draw arrows on the left and right edges to indicate that these edges are glued together. However, we have one arrow drawn up, and one arrow drawn down, to indicate that the ends of the strip are glued together with a half twist. Since the top and bottom of the Möbius strip do not get identified together at all, we do not draw any arrows on them. Then, if an edge goes off one end the Möbius strip near the top, it comes back on the opposite end near the bottom, and vice versa.  Two drawings of on the Möbius strip    Finally, we explain some heuristics about drawing graphs on surfaces, with reference to drawings of on the Möbius strip. As discussed at the end of the section of the torus, it can be useful to follow part of the reasoning used in the planarity algorithm for Hamiltonian cycles when trying to draw graphs on surfaces other than the sphere. That is, start with a large cycle in the graph -- for , with red vertices, and the blue vertices, we will use the cycle . It may be that this cycle is drawn in the plane as a circle, as in the left hand example; then we try to connect as many edges as possible through the centre of the circle, and then do the rest on the outside, possibly wrapping around the Möbius strip.  But the Jordan Curve Theorem (that a circle has an inside and an outside) only holds for the sphere -- on other surfaces, there are always curves where this isn't true. Whether of necessity, or choice, it might be that our large cycle is drawn as one of these loops that doesn't have an inside or an outside. On the right hand side of , we have drawn the cycle as the centre of the Möbius strip.  Finally, we note that in general for a surface, there are multiple different ways to draw curves; in , we have drawn our chosen Hamiltonian cycle as a curve that wraps twice about the Möbius strip. What happens if you cut a physical Möbius strip along this line?  A third drawing of on the Möbius strip     "
},
{
...
},
{
  ...
  "body": "The football theorem Let be a football graph drawn on the plane, with pentagonal faces, and hexagonal faces. Then .  Let denote the number of vertices, edges, and faces of . Since every face is a hexagon or pentagon, we have , and substituting this into Euler's formula gives:   Now we turn to handshaking. Since is trivalent, every vertex has degree three, and so vertex-edge handshaking becomes:   Finally, since there are pentagonal faces and hexagonal faces, face-edge handshaking becomes:   Multiplying by six gives:   Multiplying by two gives , which we can use to eliminate , giving   Finally, using we can eliminate both and in one go, being left with as desired.  "
},
{
...
{
  "id": "s_chromatic_number-13",
  "level": "2",
  "url": "s_chromatic_number.html#s_chromatic_number-13",
  "type": "Example",
  "number": "5.1.6",
  "title": "The wheel graph.",
  "body": "The wheel graph  The wheel graph consists of an -cycle together with one additional vertex, that is connected to all vertices of the -cycle. Note that this with this convention, confusingly has vertices; other people may use a different convention where has vertices, but then it only has vertices on the actual wheel.  Since the central vertex is connected to all other vertices, once we colour it, we can never use that colour again. But deleting that vertex we just have the -cycle, and we already know the chromatic number of that. So we have . "
},
{
  ...
  "number": "5.1.7",
  "title": "",
  "body": "We write for the maximum degree of any vertex in :  "
},
{
...
},
{
  ...
  "body": "Chromatic index  The chromatic index  of a graph is the smallest number of colours needed to colour the edges of so that any two edges that share a vertex have different colours. "
},
{
...
},
{
  ...
  "body": "Vizing's Theorem For any graph we have  "
},
{
...
{
  "id": "s_chromatic_polynomial_intro-3-7",
  "level": "2",
  "url": "s_chromatic_polynomial_intro.html#s_chromatic_polynomial_intro-3-7",
  "type": "Example",
  "number": "5.3.4",
  "title": "Trees <span class=\"process-math\">\\(T\\)<\/span>.",
  "body": "Trees For any tree , the chromatic polynomial is given by , where is the number of vertices in the tree. The proof is left as an exercise (easy induction). "
},
{
  ...
  "body": "Two triangles joined at a vertex  Consider the graph consisting of two triangles joined at a vertex, shown at the right. We can calulate by working vertex by vertex: there are ways to colour vertex 1, but then when we colour vertex 2 it can't be the same as vertex 1, and so there are ways to colour it. Vertex three is adjacent to both one and two, so there are ways to colour it, and vertex 4 is adjacent to one coloured vertex, vertex 3, so there are again ways to colour it, and finally vertex 5 is adjacent to vertices 3 and 4 and so we have ways to colour it.Putting it together, we see .    "
},
{
  "id": "s_chromatic_polynomial_intro-3-9",
  "level": "2",
  "url": "s_chromatic_polynomial_intro.html#s_chromatic_polynomial_intro-3-9",
  "type": "Example",
  "number": "5.3.6",
...
},
{
  ...
  "number": "5.3.7",
  "title": "",
  "body": "Let be a disconnected graph. Then .  A colouring of with colours gives a colouring of with colours and a colouring of with colours.  Similarly, since are disconnected, how we colour one will have no effect on what colourings are possible for the other. Hence, colouring is exactly the same as colouring and . "
},
{
  "id": "s_chromatic_polynomial_intro-4-6",
  "level": "2",
  "url": "s_chromatic_polynomial_intro.html#s_chromatic_polynomial_intro-4-6",
  "type": "Proposition",
  "number": "5.3.8",
  "title": "",
...
{
  "id": "s_chromatic_polynomial_intro-4-7",
  "level": "2",
  "url": "s_chromatic_polynomial_intro.html#s_chromatic_polynomial_intro-4-7",
  "type": "Example",
  "number": "5.3.9",
...
{
  "id": "s_chromatic_polynomial_intro-4-8",
  "level": "2",
  "url": "s_chromatic_polynomial_intro.html#s_chromatic_polynomial_intro-4-8",
  "type": "Proposition",
  "number": "5.3.10",
  "title": "",
  "body": "Suppose has two subgraphs and , so that their union is all of , but their intersection is a single edge connecting two vertices and , i.e. and . Then we have the following relation between their chromatic polynomials:    The proof is extremely similar to that of the previous proposition. A colouring of gives us colourings of and , but not any two colourings: they match at both and at .  Now, could be any of the colours, but , being adjacent to , can't be the same colour, and so it has possibilities given a choice of colour for . Thus, there are ways to colour and , and all possibilities will occur equally often within the colourings counted by . Hence, given a colouring of , there will be ways to extend that colouring to one of all of , giving us the result. "
},
{
  "id": "s_chromatic_polynomial_intro-4-9",
  "level": "2",
  "url": "s_chromatic_polynomial_intro.html#s_chromatic_polynomial_intro-4-9",
  "type": "Example",
  "number": "5.3.11",
...
},
{
  ...
  "number": "5.4.2",
  "title": "Contraction.",
  "body": "Contraction  Let be a simple graph, and let be an edge between vertices and . Then we write for the graph with contracted. More precisely, is the simple graph with vertices . Two non- vertices are adjacent in if and only if they were adjacent in , and a vertex is adjacent to in if and only if it was adjacent to or in . "
},
{
  ...
  "body": "Deletion-Contraction Let be a simple graph, and let be any edge. Then we have:     Since and have the same vertex set, it is not too difficult to compare their colourings. Any valid colouring of will give a colouring of , but not every colouring of arises this way -- in , colourings require that and , the two endpoints of , have different colours, but there is no such requirement for . So, we want to count the colourings of where have the same colour. But these are just the colourings of : given a colouring of we can get a colouring of by giving most vertices the same colour, and giving both whatever colour was. By definition, we see that and will have the same colour in this colouring; and given any colouring of where have the same colour we can get a colouring of by colouring the colour that were.  "
},
{
